/**
 * GigiAI - Core Application with Theme System
...
function updateThemeIcon(theme) {
    ...
    if (text) {
        text.textContent = themeLabels[theme];
    }
}
...
function updateStatus(status, message = '') {
    ...
    // Update input glow based on status
...
function initSession() {
    ...
    console.log(`[Session] Initialized: ${AppState.sessionId}`);
...
function handleMobileKeyboard() {
    ...
    input.addEventListener('focus', () => {
        // Add focused class for gradient glow
        const glowWrapper = input.closest('.input-glow-wrapper');
        if (glowWrapper) {
            glowWrapper.classList.add('focused');
        }
    ...
    input.addEventListener('blur', () => {
        ...
        const glowWrapper = input.closest('.input-glow-wrapper');
        if (glowWrapper) {
            glowWrapper.classList.remove('focused');
        }
    });
...
async function processRetryQueue() {
    ...
    console.log(`[Retry] Processing ${queue.length} queued items`);
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('[App] Initializing GigiAI v' + APP_CONFIG.version);
