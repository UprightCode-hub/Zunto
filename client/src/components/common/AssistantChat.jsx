...
const AssistantChat = () => {
  ...
  const [messages, setMessages] = useState([
    { text: "Hello! I'm your Zunto assistant. Please log in to start chatting with GIGI AI.", sender: 'bot' }
  ]);
  ...
  const handleSend = async (e) => {
    ...
    if (!isAuthenticated) {
      setMessages((prev) => ([
        ...prev,
        { text: 'Please log in to continue chatting with GIGI AI.', sender: 'bot' },
      ]));
      return;
    }
    ...
    try {
      ...
      if (error?.status === 401) {
        setMessages((prev) => [...prev, { text: 'Login is required to use GIGI AI.', sender: 'bot' }]);
      } else {
        console.error(error);
      ...
      {isOpen && (
        <div className="bg-white rounded-lg shadow-2xl w-80 sm:w-96 flex flex-col h-[500px] border border-gray-200">
          <div className="bg-blue-600 text-white p-4 rounded-t-lg flex justify-between items-center">
            <h3 className="font-bold">GIGI AI Assistant</h3>
          ...
          {!isAuthenticated && (
            <div className="p-3 border-b border-gray-200 bg-blue-50 text-blue-900 text-sm flex items-center justify-between gap-2">
              <span>Login required to use GIGI AI.</span>
              <button type="button" onClick={goToLogin} className="px-3 py-1.5 rounded-md bg-blue-600 text-white font-semibold hover:bg-blue-700">
                Login
              </button>
            </div>
          )}
          ...
          <form onSubmit={handleSend} className="p-3 border-t border-gray-200 bg-white rounded-b-lg flex">
            <input
              ...
              placeholder={isAuthenticated ? 'Type your message...' : 'Log in to chat with GIGI AI'}
